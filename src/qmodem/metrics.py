...
def compute_rmse(test_datasource: LabelledDataSource, model: RandomCallModel) -> float:
...
def compute_point_crps(
    test_datasource: LabelledDataSource, model: RandomCallModel
) -> float:
    ...
    keys = jax.random.split(
        jax.random.key(0), 10
    )  # TODO fix hardcoded number of samples
    ...
    for i in range(test_datasource.X.shape[0]):
        ...
        x_grid = jnp.linspace(
            ...
            num=100,  # TODO fix hardcoded number of grid points
        )
