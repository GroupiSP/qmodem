...
def test_works_with_1d_features():
...
def test_works_with_multid_features():
...
def test_alignment_target_is_rul_at_window_end():
...
def test_short_history_padded_with_first_value():
...
def test_window_shape_and_scalar_targets():
...
def test_number_of_windows(N_t, window_size, stride):
...
class TestGetTimeWindowsAndJoin:
    def test_output_shapes_1d_features(self):
    ...
    def test_output_shapes_multid_features(self):
    ...
    def test_total_window_count(self, run_lengths, window_size, stride):
        feature_cols = ["feat_a"]
        df = _make_run_df(run_lengths, feature_cols)
        X, y = get_time_windows_and_join(df, window_size, stride, features=feature_cols)
        expected = sum(
            int(np.ceil((N_t - window_size) / stride)) + 1
            for N_t in run_lengths.values()
        )
